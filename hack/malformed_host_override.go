...
// to overcome the 400 Bad request coming from old storm
...
// an old storm client which send an invalid Host header. To not
// error out in http.Server we rewrite the first bytes of the request
// to sanitize the Host header itself.
// In case we're not dealing with old storm clients the data is just passed
...
func (l *MalformedHostHeaderOverrideConn) Read(b []byte) (n int, err error) {
	...
	if l.first && len(b) == 4096 {
		...
		for i := 0; i <= c-1-7; i++ {
			...
			// ensure clients other than the storm clients do not get this hack
			if i != firstLineFeed {
				return c, nil
			}
			start = i + 7
